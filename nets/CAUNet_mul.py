...
class CAUNet(nn.Module):
    ...
    def __init__(self, input_channel=3, output_channel=1, bn_momentum=0.2, feature=False):
        ...
        self.preBlock = nn.Sequential(
            nn.Conv2d(input_channel, 24, kernel_size=3, padding=1),
        ...
        self.out_conv1 = nn.Sequential(
            nn.ConvTranspose2d(128, output_channel, kernel_size=8, stride=8),
        )
        self.out_conv2 = nn.Sequential(
            nn.ConvTranspose2d(64, output_channel, kernel_size=4, stride=4),
        )
        self.out_conv3 = nn.Sequential(
            nn.ConvTranspose2d(32, output_channel, kernel_size=2, stride=2),
        )
...
if __name__ == '__main__':
    images = torch.rand(2, 3, 640, 640).to('mps')
    model = CAUNet(3, 5).to('mps')
    output = model(images)
    print(len(output))
