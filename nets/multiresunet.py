...
def conv2d_bn(in_channels, filters, kernel_size, padding=1, activation='relu'):
    sequence = []
    sequence += [torch.nn.Conv2d(in_channels, filters, kernel_size, padding=padding, bias=False)]
    sequence += [torch.nn.BatchNorm2d(filters)]
    if activation == "relu":
...
class MultiResBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, u, alpha=1.67, use_dropout=False):
        ...
        self.conv2d_bn = conv2d_bn(in_channels, self.out_channel, 1, padding=0,activation='')
...
class ResPathBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, filters):
        ...
        self.conv2d_bn1 = conv2d_bn(in_channels, filters, 1, padding=0, activation=None)
...
class MultiResUnet(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, nf=32, use_dropout=False):
        ...
        nf0 = nf
        self.mres_block1 = MultiResBlock(in_channels, u=nf0)
        self.pool = torch.nn.MaxPool2d(kernel_size=2)
        self.res_path1 = ResPath(self.mres_block1.out_channel, nf0, 4)
        ...
        self.deconv4 = torch.nn.ConvTranspose2d(self.mres_block8.out_channel, nf0, (2, 2), (2, 2))
        self.mres_block9 = MultiResBlock(nf0 + nf0, u=nf0)
        self.conv10 = conv2d_bn(self.mres_block9.out_channel, out_channels, 1, padding=0, activation='None')
